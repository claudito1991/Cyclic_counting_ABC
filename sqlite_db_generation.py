...
def select_random_article(db_name, table_name, category):
    cnt = sqlite3.connect(db_name)
    query = cnt.execute (f""" SELECT ARTICULO, DESCRIPCION, CLASIFICACION FROM {table_name}
...
def check_if_article_has_quantity(db_name, table_name, article):
    cnt = sqlite3.connect(db_name)
    query = cnt.execute (f""" SELECT CANTIDAD FROM {table_name} WHERE ARTICULO={str(article)} """)
    rows = query.fetchone()
    if rows[0] == 0:
      return False
    else:
